###start with vegetation matrix
###rows=plot numbers X columns=vegetation variables

#creates object in R called veg.table, and attaches it

    veg.table <- read.table("I:\\GitHub\\Vegetation_Classification\\BECMasterVeg_Oct26_2018.txt",header=TRUE)
    attach(veg.table)

#produces a summary of veg.table

    summary(veg.table)

#plots all data, saves it

    pdf (file = "I:\\GitHub\\Vegetation_Classification\\BECMasterVeg_Oct26_2018.PDF")
    plot(veg.table)
    dev.off()
