...
SUcovData2 <- melt(SUcovData, id.vars = c("SiteSeries","Species"))
SScovMatrix <- dcast(SUcovData2, SiteSeries ~ Species + variable)###Convert to site unit by species matrix
SScovMatrix[is.na(SScovMatrix)] <- 0
rownames(SScovMatrix) <- gsub(" ", "",SScovMatrix$SiteSeries)
...
SS.List <- SS.List[,c(7,11)] ## select by hierarchy level
SUsumMatrix <- merge(SS.List, SSsumMatrix, by = "SiteSeries")
SUsumMatrix[is.na(SUsumMatrix)] <- 0
SUsumMatrix$Association <- as.factor(SUsumMatrix$Association)
